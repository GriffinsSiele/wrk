...
def level_label(level_num: int) -> str:
    return {
        ...
        2: "Level 2 — Recovery Intelligence Coach",
        3: "Level 3 — Performance Systems Coach",
...
async def has_passed_practical(
    db: AsyncSession, user_id: int, level: str
) -> Tuple[bool, Optional[PracticalAssessment]]:
    target = normalize_level(level)
    result = await db.execute(
        select(PracticalAssessment)
        .where(
            PracticalAssessment.user_id == user_id,
...
async def agreements_fully_signed(db: AsyncSession, user_id: int) -> bool:
    result = await db.execute(
        select(CoachAgreement).where(
            CoachAgreement.user_id == user_id,
...
async def try_issue_certificate(
    db: AsyncSession,
    user_id: int,
    ...
    # Avoid duplicate active cert for same level
    ...
    cert = Certificate(
        ...
        certification_level=level_label(target) if target else certification_level,
