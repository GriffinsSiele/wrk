"""
Rich Phase-1 demo seed for Olynixx Academy portals.
...
async def seed_data(force: bool = False) -> None:
    async with AsyncSessionLocal() as session:
        ...
        # ── Course content ─────────────────────────────────────
        course = Course(
            title="Human Readiness Certification (Level 1)",
            description="The foundational course for Olynixx coaching — physiology of readiness and recovery.",
            is_published=True,
        ...
        modules_spec = [
            ...
            ("Module 2: Recovery Intelligence", [
        ...
        past_session = ExamSession(
            title="Level 1 Online Written — March Cohort",
            date=days_ago(21),
            is_online=True,
            location="Online",
            capacity=40,
        ...
        upcoming_session = ExamSession(
            title="Level 1 Online Written — July Cohort",
        ...
        session.add(ExamRegistration(user_id=john.id, session_id=upcoming_session.id, status=ExamStatus.BOOKED, registered_at=days_ago(3)))
        ...
        session.add_all([maya_attempt, omar_attempt, nina_attempt, pending_attempt])
        await session.flush()
        ...
        session.add(
            PracticalAssessment(
                user_id=learners["james@olynixx.com"].id,
                assessor_id=admin.id,
                certification_level="Level 1",
        ...
        for name, industry, emirate, email in operators_spec:
            ...
            await session.execute(
                text("SELECT setval(pg_get_serial_sequence('clients', 'id'), COALESCE((SELECT MAX(id) FROM clients), 1), true)")
            )
            operators.append(op)
        ...
        for op_idx, title, ptype, status, created in projects_spec:
            proj = Project(
                operator_id=operators[op_idx].id,
            ...
            await session.execute(
                text("UPDATE projects SET client_id = :cid WHERE id = :pid"),
